...
void Deer::Process::GetEnv(V8_ARGS) {
    V8_ISOLATE
...
void Deer::Process::Fork(V8_ARGS) {
    V8_ISOLATE
    V8_RETURN(Number::New(isolate, fork()));
}

void Deer::Process::Wait(V8_ARGS) {
    int status;
    wait(&status);
}

void Deer::Process::Cwd(V8_ARGS) {
    V8_ISOLATE
    char * cwd = getcwd(nullptr, 0);
    V8_RETURN(newStringToLcal(isolate, cwd));
}

void Deer::Process::Exit(V8_ARGS) {
    int status = 0;
    if (args.Length() > 0) {
...
void Deer::Process::Execve(V8_ARGS) {
    V8_ISOLATE
...
void Deer::Process::Init(Isolate* isolate, Local<Object> target) {
  ...
  setMethod(isolate, process, "getEnv", Deer::Process::GetEnv);
  setMethod(isolate, process, "fork", Deer::Process::Fork);
  setMethod(isolate, process, "wait", Deer::Process::Wait);
  setMethod(isolate, process, "cwd", Deer::Process::Cwd);
  setMethod(isolate, process, "exit", Deer::Process::Exit);
  setMethod(isolate, process, "execve", Deer::Process::Execve);
