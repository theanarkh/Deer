...
void Deer::VM::CompileFunction(V8_ARGS) {
    V8_ISOLATE
    V8_CONTEXT
    String::Utf8Value str(isolate, args[0].As<String>());
    Local<Object> options = args[1].As<Object>();
    ...
    int names_len = 0;
    std::vector<Local<String>> name_vector;
    ...
    int exts_len = 0;
    std::vector<Local<Object>> extension_vector;
    ...
    if (fun.IsEmpty()) {
      ...
      if (call.As<Boolean>()->Value()) {
        ...
        int argc = 0;
        std::vector<Local<Value>> values_vector;
...
void Deer::VM::Init(Isolate* isolate, Local<Object> target) {
  ...
  setMethod(isolate, vm, "run", Deer::VM::Run);
  setMethod(isolate, vm, "compileFunction", Deer::VM::CompileFunction);
